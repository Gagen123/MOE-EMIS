...
<div>
    <table id="studenthealth-table" class="table table-bordered text-sm table-striped">
            <thead>
                <tr>
                    <th>SL#</th>
                    <th>Health Screening Type</th>
                    <th>Status</th>
                    <th>Action</th> 
                </tr>
            </thead>
            <tbody id="tbody">
                <tr v-for="(item, index) in screeningList" :key="index">
                    <td>{{ index + 1 }}</td>
                    <td>{{ item.name}}</td>
                    <td>{{ item.status==  1 ? "Active" : "Inactive" }}</td>
                    <td>
                        <div class="btn-group btn-group-sm">
                            <a href="#" class="btn btn-info" @click="viewScreeningList(item)"><i class="fas fa-edit"></i ></a>
...
export default {
    ...
    methods:{
        loadScreeningList(uri = 'masters/loadScreening'){
            axios.get(uri)
            .then(response => {
                let data = response;
                this.screeningList =  data.data;
            })
