...
class SpmsMasterController extends Controller
{
    ...
    public function saveSpmMasters(Request $request)
    {
        ...
        if ($request['record_type'] == 'indicator') {
            ...
            if ($request['action_type'] == "edit") {
                ...
                $messs_det = 'sequence_no:' . $data->sequence_no . 'name:' . $data->name . 'weighting:' . $data->weighting . 'mov:' . $data->mov . '; spm_parameter_id:' . $data->spm_parameter_id . '; status' . $data->status;
    ...
    public function loadSpmMasters($param = "")
    {
        ...
        if ($param == "all_indicators") {
            $indicators = DB::select("SELECT t1.sequence_no,trim(t1.weighting)+0 AS weighting,CONCAT(t4.sequence_no,'.',t3.sequence_no,'.',t2.sequence_no,'.',t1.sequence_no) AS display_sequence_no,t1.id,t3.spm_domain_id,t2.spm_area_id,t1.spm_parameter_id,t4.name AS domain,t3.name AS area,t2.name AS parameter,t1.name,t1.mov,t1.status FROM spm_indicator t1 
        ...
        if (strpos($param, 'all_active_indicators_') !== false) {
            $params = explode("_", $param);
            $indicators = DB::select('SELECT t1.sequence_no,trim(t1.weighting)+0 AS weighting,t1.id,t3.spm_domain_id,t2.spm_area_id,t1.spm_parameter_id,t4.name AS domain,t3.name AS area,t2.name AS parameter,t1.name,t1.status FROM spm_indicator t1 
            JOIN spm_parameter t2 ON t1.spm_parameter_id = t2.id 
