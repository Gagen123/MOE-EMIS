                ...
                if($type=="commonLeaveOthers"){
                    ...
                    foreach($screen_status as $i => $srcn){
                        if(isset($srcn['Establishment_type'])){
                            ...
                            $result_data.='( t.screen_id="'.$srcn['SysSubModuleId'].'" AND LOWER(t.service_name)="'.$srcn['Establishment_type'].'" AND t.name IN ("Change in Name","Upgrade Downgrade") AND t.status_id='.($srcn['Sequence']-1).')'; 
                            if(sizeof($screen_status)-1==$i){
                                $result_data.=')'; 
                            } 
                            else{ 
