    ...
    <div>
        <form @submit.prevent="save" class="bootbox-form">
            ...
            <div class="form-group row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <label>Remarks:</label> 
                    <textarea v-model="form.remarks" name="remarks" id="time_description" class="form-control form-control-sm"></textarea>
                </div>
            </div>
    ...
    export default {
        ...
        methods:{
            ...
            async getStudents(){
              ...
              let students = await axios.get(uri)
                .then(response => {
               ...
               let getRemedialClassDetail = await axios.get('academics/getRemedialClassDetail/'+this.form.id).then(response => response.data.data);
               students.forEach((item,index) => {
                   getRemedialClassDetail.forEach(item1 => {
                       if(item.std_student_id == item1.std_student_id){
                           this.form.std_student_id.push(item1.std_student_id)
                       }
                   })
               })
        ...
        created(){
            this.class_stream_section_id[0] = this.$route.params.data.OrgClassStreamId;
