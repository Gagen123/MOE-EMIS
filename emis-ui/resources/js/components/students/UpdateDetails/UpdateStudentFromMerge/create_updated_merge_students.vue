<template>
    <div>
        ...
        <form class="bootbox-form" id="mainform">
           ...
           <label><u>Allocating the student to each organization which has been Bifcurcated:</u><br><small><i>(Allocate to one organization)</i></small></label><br>
                    ...
                    <div class="table-responsive overflow-auto">
                        <table id="priveleges_table" class="table table-sm table-bordered table-striped" style="width: 100%;">
                            <thead class="table-secondary">
                                <tr>
                                    <th>Lists of students</th>
                                    <th>{{this.form.bifuractionOrg}}</th>
                                    <th>{{this.form.parentOrg}}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <template v-for="(student, key) in studentList">
                                    <tr :key="key">
                                        <td><b><i>{{student.Name}}</i></b></td>
                                        <td><input type="radio" name="name" :value="student.id+'__'+form.bifuractionOrg"></td>
                                        <td><input type="radio" name="name" :value="student.id+'__'+form.parentOrg"></td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        ...
        loadParentOrgDetailOfBifcurcation(id){
            ...
            .then(response => {
                let data = response.data;
                this.loadStudentListByOrgId(data[0].orgId)
           })
            .catch(function (error){
                console.log("Error: "+error)
            });
        },
        loadStudentListByOrgId(org_id){
             axios.get('common/loadStudentListByOrgId/'+org_id)
            .then(response => {
                let data = response.data;
