<template>
    <div> 
        <div class="invoice p-3 mb-6">
            <div class="row">
                <div class="col-12">
                    <h5>Health Vaccination Details</h5>
                </div>
            </div>
            <div class="row invoice-info">
                <div class="col-sm-10 invoice-col">
                    <div class="table-responsive">
                        <table v-for="(item, index) in vaccinationDetails" :key="index">
                        <tr>
                            <th style="width:50%">Vaccination Type:</th>
        ...
        <div class="row">
            <div class="col-12">
                <h6>Student List and Vaccination Details</h6>
            </div>
        </div>
            ...
            <table id="list-screening" class="table table-sm table-bordered table-striped">
                ...
                <tbody id="tbody">
                    <tr v-for="(item, index) in studentList" :key="index">
                        ...
                        <td>{{ item.vaccinated ==  null ? "Vaccinated" : "Not Vaccinated" }}</td>
                        <td>
                            ...
                            <form class="bootbox-form">
                                ...
                                <div class="row form-group rp=0.5">
                                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                                        ...
                                        <label><input  type="radio" v-model="student_form.vaccinated" value="1" tabindex=""/> Yes</label>
                                        <label><input  type="radio" v-model="student_form.vaccinated" value="0" tabindex=""/> No</label>
                                    </div>
                                </div>
                            </form>
                        </div>
...
export default {
   data(){
    ...
    methods: {
        ...
        */
        getHealthVaccinationDetails(id){
            axios.get('students/getHealthVaccinationDetails/'+id)
            .then((response) => {  
                this.vaccinationDetails = response.data;  
            })
            .catch((error) =>{  
         ...
         */
        getStudentList(id){
            axios.get('/students/loadViewVaccinationDetails/'+id)
                    .then((response) => {
        ...
        showaddmodal(id){
            ...
            axios.get('/students/getVaccinationDetails/'+id)
                    .then((response) => {
                        this.screeningDetails = response.data;
                        
                        this.student_form.id = response.data[0].vaccination_id;
                        this.student_form.StdHealthVaccinationId = response.data[0].id;
                        this.student_form.StdStudentId = response.data[0].StdStudentId;

                        if(response.data[0].vaccinated==='vaccinated'){
                            this.student_form.vaccinated=1;
                        }
                        else{
                            this.student_form.vaccinated=0;
                        }
                })
                .catch(() => {
                    consoele.log("Error:"+e)
        ...
        addMore: function(type){
            this.student_form.post('/students/updateHealthVaccinationRecords',this.student_form)
            .then((response) => {  
                Toast.fire({
                    icon: 'success',
    ...
    created() {
        ...
        this.getHealthVaccinationDetails(this.$route.params.data.id);
