...
export default {
    ...
    mounted() {
        ...
        this.form.inventoryId=this.$route.params.data.CeaProgrammeInventoryId;
        let uri='students/loadInventoryDetials/'+this.form.inventoryId;
        axios.get(uri)
        .then(response =>{
            let data = response.data.data;
            this.inventoryDetials =  data;
            this.form.program=data.CeaProgrammeId;
            $('#program').val(data.CeaProgrammeId).trigger('change');
            this.form.month=data.ForMonth;
            if(data.details && data.details.length>0){
                data.details.forEach(itm => {
                    this.form.inventoryDetails.push({
                        id:itm.id,item_id:itm.CeaProgrammeInventoryItemId, previous_balance:itm.PreviousQty, increase_quantity:itm.IncreaseInQuantity, decrease_quantity:itm.DecreaseInQuantity,measurement_unit:'', remarks:itm.Remarks,
                    })
                });
            }
            if(data.production && data.production.length>0){
                data.production.forEach(itm => {
                    this.form.productionDetails.push({
                        id:itm.id,
                        item_produced:itm.QuantityProduced,
                        quantity_produced:itm.NoOfVariety,
                        quantity_unit:'',
                        no_varieties:itm.NoOfVariety,
                        amount_generated:itm.AmountGenerated,
                        production_remarks:itm.Remarks
                    });
                });
            }
            if(data.expenditure && data.expenditure.length>0){
                data.expenditure.forEach(itm => {
                    this.form.expenditureDetails.push({
                        expenditure_details:itm.Particular, expenditure_amount:itm.Amount,expenditure_remarks:itm.Remarks
                    })
                });
            }
        })
