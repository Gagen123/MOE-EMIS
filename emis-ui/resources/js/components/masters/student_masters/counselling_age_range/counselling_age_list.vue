<template>
    <div>
        <table id="age-range-table" class="table table-bordered text-sm table-striped">
            <thead>
                <tr>
                    <th >SL#</th>
                    <th >Age Range</th>
...
export default {
    ...
    methods:{
        loadcounsellingList(uri = 'masters/loadStudentMasters/CounsellingAgeRange'){
            axios.get(uri)
            .then(response => {
                let data = response;
        ...
        showedit(data){
            this.$router.push({name:'counselling_age_edit',params: {data:data}});
        },
    },
    mounted(){
