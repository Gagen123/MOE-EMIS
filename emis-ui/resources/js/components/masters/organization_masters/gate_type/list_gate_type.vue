        ...
        <table id="org-masters-table" class="table table-bordered text-sm table-striped">
            <thead>
                <tr>
                    ...
                    <th style="width:20%">Gate Type</th>
...
export default {
    ...
    methods:{
        loadContactList(uri = 'masters/organizationMasterController/loadOrganizaitonmasters/all/GateType'){
            axios.get(uri)
            .then(response => {
        ...
        editmasters(data){
            this.$router.push({name:'edit_gate_type',query: {data:data}});
        },
    },
    mounted(){
