<template>
    <div>
        <form class="bootbox-form" id="electricitySourceId">
            <div class="card-body">
                <div class="row form-group">
                    <input type="hidden" class="form-control" v-model="form.id"/>
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                        <label>Source Name:<span class="text-danger">*</span></label> 
                        <input class="form-control" v-model="form.sourceName" :class="{ 'is-invalid': form.errors.has('sourceName') }" id="sourceName" @change="remove_err('sourceName')" type="text" tabindex="1" autofocus="true">
                        <has-error :form="form" field="sourceName"></has-error>
                    </div>
...
export default {
    ...
    methods:{
        ...
        formaction: function(type){
            ...
            if(type=="save"){
                this.form.post('masters/saveElectricitySource',this.form)
                    .then(() => {
                    Toast.fire({
                        icon: 'success',
                        title: 'Electricity source details is updated successfully'
                    })
                    this.$router.push('/electricity_source_list');
                })
                .catch(() => {
                    console.log("Error......")
                })
    ...
    created() {
        this.form.sourceName=this.$route.params.data.name;
