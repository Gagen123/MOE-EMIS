<template>
    <div>
        <table id="working-agency-table" class="table table-bordered text-sm table-striped">
            ...
            <tbody id="tbody">
                <tr v-for="(item, index) in workingagnecyList" :key="index">
                    <td>{{ index + 1 }}</td>
                    <td>{{ item.name}}</td>
...
export default {
    ...
    methods:{
       loadworkingagencyList(uri = 'masters/loadStaffMasters/all_workingagency'){
            ...
            .then(response => {
                let data = response;
                this.workingagnecyList =  data.data.data;
            })
            .catch(function (error) {
                if(error.toString().includes("500")){
                    $('#tbody').html('<tr><td colspan="5" class="text-center text-danger text-bold">This server down. Please try later</td></tr>');
                }
            });
        },
        showedit(data){
            this.$router.push({name:'edit_workingagency',params: {data:data}});
        },
    },
    mounted(){
