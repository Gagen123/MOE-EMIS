        ...
        <div class="card card-primary card-outline" id="mainform">
            <div class="card-header pt-0 mt-0 pb-0">
                <span class="card-title pt-2 mb-0">
                        <b>Applications for New ECR Establishment</b>
                </span>
                <span class="fa-pull-right pr-2 py-1">
                    <button type="button" class="btn btn-primary text-white btn-sm" @click="showadprocess('list_ecr')"><i class="fa fa-list"></i> List</button>
                    <button type="button" class="btn btn-dark text-white btn-sm" @click="showadprocess('create_ecr')"><i class="fa fa-plus"></i> Apply for New Establishment</button>
...
export default {
    ...
    mounted() {
        axios.get('common/getScreenAccess/workflow__establishment_of_public_ecr')//workflow will specify that the process have workflow
        .then(response => {
