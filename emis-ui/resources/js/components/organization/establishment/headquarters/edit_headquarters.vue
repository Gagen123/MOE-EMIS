...
export default {
    data(){
        return{ 
            ...
            form: new form({
                ...
                gewog:'',chiwog:'',agencyType:'',status:'Active',department:'',action_type:'edit'
            }),
    ...
    methods: {
        ...
        async getvillagelist(id,village_id){
            ...
            .then(response =>{
                ...
                this.form.chiwog=village_id;
                $('#chiwog').val(village_id).trigger('change');
            })
            .catch(function (error){
                console.log("Error:"+error)
            });
         ...
         */
         loadDetails(id){
            axios.get('organization/loadheadQuarterDetails/'+id)
            .then((response) => {  
                ...
                $('#department').val(response.data.data.department).trigger('change');
                ...
                if(data.contact.length>0){
                    for(let i=0;i<data.contact.length;i++){
                        this.form1.contacts.push({names:data.contact[i].contactTypeId,phone:data.contact[i].phone,mobile:data.contact[i].mobile,email:data.contact[i].email});
                    }
                }
            })
            .catch((error) => {  
    ...
    mounted() {
        ...
        this.loadDetails(this.$route.query.data);
    },
}
</script>