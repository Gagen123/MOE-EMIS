                    ...
                    <tbody>
                        <tr v-for="(item, index) in data_list" :key="index">
                            ...
                            <td>
                                ...
                                <a href="#" v-if="item.status===1" class="btn btn-info btn-sm btn-flat text-white" @click="loadeditpage(item,'view')"> <span class="fa fa-edit"></span> Edit</a>
...
export default {
    ...
    methods:{
        loadeditpage(itme){
            this.$router.push({name:"edit_transfer_cancellation_order",query:{data:itme}});
        },
        loadviewpage(itmem,type){
            this.$router.push({name:"view_transfer_cancellation_order",query:{data:itmem,type:type}});
        },
    },
    mounted(){
        axios.get('/staff/staffOfficeorderController/loadCreatedApplication/TransferOrder__TransferCancel')
        .then((response) =>{
