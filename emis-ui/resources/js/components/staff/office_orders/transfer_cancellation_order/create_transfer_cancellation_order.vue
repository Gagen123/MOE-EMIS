...
export default {
     data(){
        return {
            ...
            form: new form({
                ...
                reason:'',
                remarks:'',
                office_order_type:'TransferCancel',
                model:'TransferOrder',
                action_type:'add'
            }),
        }
                    ...
                    .then((response) => {
                        if(response.data!=""){
                            if(response!=""){
                                ...
                                this.$router.push({name:'transfer_cancellation_order_acknowledgement',params: {data:res }});
                                Toast.fire({
                                    icon: 'success',
    ...
    async mounted(){
        axios.get('/staff/staffOfficeorderController/checkOngoingApplication/TransferOrder__TransferCancel/')
        .then((response) =>{
            let data=response.data.data;
            if(data!=null && data!=""){
                let res='You have already submitted request for this service with order number : '+data.order_no+'.<br><b>Thank You !</b>';
                this.$router.push({name:'transfer_order_acknowledgement',params: {data:res }});
            }else{
