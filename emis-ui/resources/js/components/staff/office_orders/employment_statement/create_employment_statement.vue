...
export default {
                    ...
                    .then((response) => {
                        if(response.data!=""){
                            if(response!=""){
                                ...
                                this.$router.push({name:'employment_statement_acknowledgement',params: {data:res }});
                                Toast.fire({
                                    icon: 'success',
    ...
    async mounted(){
        axios.get('/staff/staffOfficeorderController/checkOngoingApplication/TransferOrder__EMPSTAT/')
        .then((response) =>{
