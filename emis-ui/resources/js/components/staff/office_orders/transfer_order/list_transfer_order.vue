...
export default {
    ...
    methods:{
        loadeditpage(itme){
            this.$router.push({name:"edit_transfer_order",query:{data:itme}});
        },
        loadviewpage(itmem,type){
            this.$router.push({name:"view_transfer_order",query:{data:itmem,type:type}});
        },
    },
    mounted(){
        axios.get('/staff/staffOfficeorderController/loadCreatedApplication/TransferOrder__Transfer')
        .then((response) =>{
