...
export default {
    ...
    methods:{
        loadeditpage(itme){
            this.$router.push({name:"edit_noc",query:{data:itme}});
        },
        loadviewpage(itmem,type){
            this.$router.push({name:"view_noc",query:{data:itmem,type:type}});
        },
    },
    mounted(){
        axios.get('/staff/staffOfficeorderController/loadCreatedApplication/TransferOrder__NOC')
        .then((response) =>{
